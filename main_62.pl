...
% Testing values.
getColors(C) :- C = [[1, rosu], [2, negru], [3, albastru], [4, negru]].
...
%% TODO: use of colors, apply formula
...
getPathAux(Dest, G, Queue, Visited, Parents, PrtsRez) :-
    ...
    getPathAux(Dest, G, Q2, V, NewPrts, PrtsRez), !.
...
% assert(Term) -- add to knowledge base
% retract(Term) -- remove from knowledge base
